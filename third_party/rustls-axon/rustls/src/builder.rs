...
impl<S: ConfigSide> ConfigBuilder<S, WantsVersions> {
    ...
    pub fn with_protocol_versions(
        ...
        if self.provider.kx_groups.is_empty() {
            // TLS 1.3 external-PSK-only configurations deliberately have no
            // key-exchange groups. The handshake still fails closed unless an
            // external PSK is configured and selected.
            if versions
                .iter()
                .all(|version| version.version == ProtocolVersion::TLSv1_3)
            {
