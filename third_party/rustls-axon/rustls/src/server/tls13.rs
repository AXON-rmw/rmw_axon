...
mod client_hello {
    ...
    pub(in crate::server) struct CompleteClientHelloHandling {
        ...
        pub(in crate::server) external_psk: Option<(usize, crate::external_psk::ExternalPsk)>,
    }
    ...
    impl CompleteClientHelloHandling {
        ...
        pub(in crate::server) fn handle_client_hello(
            ...
            selected_kxg: Option<&'static dyn SupportedKxGroup>,
            ...
            let chosen_share_and_kxg = selected_kxg.and_then(|selected_kxg| {
                shares_ext.iter().find_map(|share| {
                    (share.group == selected_kxg.name()).then_some((share, selected_kxg))
                })
            });

            if let (Some(selected_kxg), None) = (selected_kxg, chosen_share_and_kxg) {
            ...
            let mut external_psk_secret = None;
            ...
            if let Some(psk_offer) = &client_hello.preshared_key_offer {
                ...
                if let Some((index, external_psk)) = self.external_psk.take() {
                    if index >= psk_offer.binders.len()
                        || !self.check_binder(
                            self.suite,
                            chm,
                            external_psk.secret(),
                    ...
                    chosen_psk_index = Some(index);
                    external_psk_secret = Some(external_psk);
                }

                let now = self.config.current_time()?;
                for (i, psk_id) in psk_offer.identities.iter().enumerate() {
                    if external_psk_secret.is_some() {
                        break;
                    }
            ...
            let external_psk_selected = external_psk_secret.is_some();
            let required_mode_offered = client_hello
                .preshared_key_modes
                .as_ref()
                .is_some_and(|offer| {
                    if external_psk_selected {
                        offer.psk
                    } else {
                        offer.psk_dhe
                    }
                });
            if !required_mode_offered {
                debug!("Client unwilling to resume, PSK_DHE_KE not offered");
                self.send_tickets = 0;
                chosen_psk_index = None;
                resumedata = None;
                external_psk_secret = None;
            } else if external_psk_selected {
                self.send_tickets = 0;
            } else {
            ...
            let full_handshake = resumedata.is_none() && external_psk_secret.is_none();
            self.transcript.add_message(chm);
    ...
    fn emit_server_hello(
        ...
        share_and_kxgroup: Option<(&KeyShareEntry, &'static dyn SupportedKxGroup)>,
        chosen_psk_idx: Option<usize>,
        resuming_psk: Option<&[u8]>,
        config: &ServerConfig,
    ) -> Result<KeyScheduleHandshake, Error> {
        let (server_key_share, shared_secret) = match share_and_kxgroup {
            Some((share, kxgroup)) => {
                ...
                (Some(KeyShareEntry::new(ckx.group, ckx.pub_key)), ckx.secret)
            }
            None => (None, crate::crypto::SharedSecret::from(&[][..])),
        };
