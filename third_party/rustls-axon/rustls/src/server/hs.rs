...
impl ExpectClientHello {
    ...
    pub(super) fn with_certified_key(
        ...
        let external_psk = if version == ProtocolVersion::TLSv1_3
            && client_hello
                .preshared_key_modes
                .as_ref()
                .is_some_and(|modes| modes.psk)
        {
            self.config
                .external_psk_resolver
                .as_ref()
                .and_then(|resolver| {
                    client_hello
                        .preshared_key_offer
        ...
        let (suite, skxg) = if external_psk.is_some() {
            let suite = self
                .config
                .provider
                .cipher_suites
                ...
                .find(|suite| {
                    matches!(suite, SupportedCipherSuite::Tls13(_))
                        && suite.usable_for_protocol(cx.common.protocol)
                        && client_hello
                            .cipher_suites
        ...
        match suite {
            ...
            .handle_client_hello(
                ...
                m,
                client_hello,
                skxg.expect("TLS 1.2 always requires a key exchange group"),
                sig_schemes,
                tls13_enabled,
            ),
        }
