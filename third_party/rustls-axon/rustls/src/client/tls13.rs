...
pub(super) fn handle_server_hello(
    ...
    let using_external_psk = resuming_session
        .as_ref()
        .is_some_and(|session| session.is_external_psk());
    ...
    let shared_secret = if using_external_psk {
        if server_hello.preshared_key != Some(0) || server_hello.key_share.is_some() {
        ...
        let our_key_share = KeyExchangeChoice::new(
            &config,
            cx,
            our_key_share.ok_or(PeerMisbehaved::MissingKeyShare)?,
            their_key_share,
        )
        .map_err(|_| {
            cx.common.send_fatal_alert(
...
pub(super) fn fill_in_psk_binder(
    ...
    let real_binder = if resuming.is_external_psk() {
...
pub(super) fn prepare_resumption(
    ...
    let obfuscated_ticket_age = if resuming_session.is_external_psk() {
        0
    } else {
        resuming_session.obfuscated_ticket_age()
    };
