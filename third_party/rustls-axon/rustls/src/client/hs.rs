...
fn emit_client_hello_for_retry(
    ...
    let mut exts = Box::new(ClientExtensions {
        ...
        named_groups: if config.external_psk.is_some() {
            None
        } else {
            Some(
                config
    ...
    if supported_versions.tls13 {
        exts.preshared_key_modes = Some(PskKeyExchangeModes {
            psk: config.external_psk.is_some(),
            psk_dhe: config.external_psk.is_none(),
        });
    }
...
impl State<ClientConnectionData> for ExpectServerHello {
        ...
        Self: 'm,
    {
        ...
        match suite {
            SupportedCipherSuite::Tls13(suite) => tls13::handle_server_hello(
                cx,
                server_hello,
                randoms,
                suite,
    ...
    fn retrieve(
        ...
        if let Some(psk) = &config.external_psk {
            ...
            let suite = config
                ...
                .cipher_suites
                .iter()
                .find_map(|suite| suite.tls13())?;
            return Some(persist::Retrieved::new(
                Self::Tls13(persist::Tls13ClientSessionValue::new_external_psk(
                    suite,
                    psk.identity(),
                    psk.secret(),
                    &config.verifier,
